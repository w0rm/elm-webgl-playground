...
planet =
  let
    ...
    triangles = divideSphere 5 octahedron
    randomTriangles = Random.step (surface 80 triangles) (Random.initialSeed 1) |> fst
  in
...
randomVec3 =
  ...
  `Random.andThen`
  (\v -> if length v < 1 then succeed v else randomVec3)
...
divide (v0, v1, v2) =
  let
    a = add v0 v2 |> normalize
    b = add v0 v1 |> normalize
    c = add v1 v2 |> normalize
  in
